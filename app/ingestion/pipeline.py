...
from app.ingestion.metadata_extractor import infer_document_type
from app.ingestion.chunker import chunk_markdown_document


def run_ingestion(file_path: str) -> IngestionResponse:
    raw_doc = load_pdf(file_path)
    parsed_pages = parse_pages(raw_doc["pages"])
    markdown_text = pages_to_markdown(parsed_pages)

    doc_id = str(uuid.uuid4())
    document_type = infer_document_type(raw_doc["file_name"], markdown_text)
    ...
    processed_path = processed_dir / f"{doc_id}.md"
    ...
    metadata_path.write_text(
        document_record.model_dump_json(indent=2),
        encoding="utf-8",
    )
